...
import { commands, Disposable, ExtensionContext, StatusBarItem, TextEditor, Uri, window, workspace } from 'coc.nvim'
...
import { getLombokVersion, isLombokImported, LombokVersionItemFactory } from './lombokSupport'
...
class RuntimeStatusBarProvider implements Disposable {
  ...
  public async initialize(context: ExtensionContext): Promise<void> {
    ...
    if (!supportsLanguageStatus()) {
      (this.statusBarItem as any).command = StatusCommands.configureJavaRuntimeCommand
    }
  ...
  private findOwnerProject(uri: Uri): string {
    ...
    const isInWorkspaceFolder: boolean = !!workspace.getWorkspaceFolder(uri.toString())
    for (const projectPath of this.javaProjects.keys()) {
  ...
  private async updateItem(context: ExtensionContext, textEditor: TextEditor | undefined): Promise<void> {
    let doc = textEditor?.document
    let fileName = doc ? Uri.parse(doc.uri).fsPath : undefined
    if (!textEditor || !fileName || path.extname(fileName) !== ".java" && !supportsLanguageStatus()) {
