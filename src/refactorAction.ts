...
function registerApplyRefactorCommand(languageClient: LanguageClient, context: ExtensionContext): void {
  context.subscriptions.push(commands.registerCommand(javaCommands.RENAME_COMMAND, async (position: RenamePosition) => {
    try {
      ...
      const renamePosition: Position = document.textDocument.positionAt(position.offset)
  ...
  context.subscriptions.push(commands.registerCommand(javaCommands.APPLY_REFACTORING_COMMAND, async (command: string, params: CodeActionParams, commandInfo: any) => {
    if (command === 'extractVariable'
      ...
      if (command === 'extractField') {
        if (!params || !Range.is(params.range)) {
          return
        }
