...
export class FileLogger extends AbstractLogger {
  ...
  private _log(level: LogLevel, scope: string, args: any[], time: string): void {
    if (this.useConsole) {
      ...
      console[method](`${stringifyLogLevel(level)}`, format(args, null, true))
    } else {
      ...
      this.promise = this.promise.then(() => {
        let fn = async () => {
          ...
          if (this.config.userFormatters !== false) {
            let parts = [time, stringifyLogLevel(level), `(pid:${process.pid})`]
...
export function initializeLogFile(filename: string) {
  logger = new FileLogger(filename, LogLevel.Trace, {
    color: false,
    userFormatters: true
  })
}
