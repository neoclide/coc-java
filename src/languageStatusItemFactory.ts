...
export function supportsLanguageStatus(): boolean {
    return typeof languages['createLanguageStatusItem'] === 'function'
}
...
export namespace ServerStatusItemFactory {
    export function create(): any {
        if (supportsLanguageStatus()) {
            const item = languages['createLanguageStatusItem']("JavaServerStatusItem", languageServerDocumentSelector)
...
export namespace RuntimeStatusItemFactory {
    export function create(text: string, vmInstallPath: string): any {
        if (supportsLanguageStatus()) {
            const item = languages['createLanguageStatusItem']("javaRuntimeStatusItem", languageServerDocumentSelector)
...
export namespace BuildFileStatusItemFactory {
    export function create(buildFilePath: string): any {
        if (supportsLanguageStatus()) {
            ...
            const item = languages['createLanguageStatusItem']("javaBuildFileStatusItem", languageServerDocumentSelector)
            // item.severity = vscode.LanguageStatusSeverity?.Information
    ...
    function getOpenBuildFileCommand(buildFilePath: string): Command {
        ...
        return {
            ...
            arguments: [Uri.file(buildFilePath)],
        }
    }
}
